...
/* ============== */
{
  ...
  while (!feof(fp)) {
    ...
    if (departure > time && arrival+delay < time)
    {
      serviceing_num ++;
    }
    if (arrival+delay > time && arrival < time)
    {
      wait_num ++;
    }
    sum.delay   += delay;
  ...
  printf("   number of jobs in the queue waiting at time = 400............ = %d\n", wait_num);
  printf("   number of jobs executing at time = 400............ = %d\n", serviceing_num);
  printf("   number of jobs in the node at time = 400............ = %d\n", service_num);
  printf("   proportion of jobs delayed.. =  %0.2f\n", delay_job_num/total_job);
